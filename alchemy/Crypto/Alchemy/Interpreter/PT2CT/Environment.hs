...
-- during compilation.
data PT2CTAux = Aux  deriving (Show)

newtype PT2CTMon a = PT2CTMon (State PT2CTAux a) deriving (Monad)

runPT2CTMon :: PT2CTMon a -> a
runPT2CTMon (PT2CTMon a) = evalState a newPT2CTAux
...
instance MonadAccumulator Keys PT2CTMon where
  ...
  accumulate f = PT2CTMon $ \(PT2CTAux keys hints) ->
    let (a,keys') = f keys
    in (a, PT2CTAux keys' hints)
