...
newtype PT2CTMon a = PT2CTMon (State (Keys, Hints) a) deriving (Functor, Applicative, Monad)

runPT2CTMon :: PT2CTMon a -> a
runPT2CTMon (PT2CTMon a) = evalState a (mempty, mempty)
...
instance MonadAccumulator Keys PT2CTMon where
  ...
  accumulate f = PT2CTMon $ state $ \(keys, hints) ->
    let (a,keys') = f keys
