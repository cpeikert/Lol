...
-- | Ciphertext over \( R'_q \) encrypting a
...
-- | Get degree of a ciphertext. 
degree :: ZeroTestable.C r'q => CT m zp r'q -> Maybe Int
degree (CT _ _ _ c) = P.degree c
...
keySwitchQuadCirc hint ct =
  ...
  in case coeffs c of
       ...
       -- this should never happen due to the type of the input
...
--  Embedding *essentially* maintains the invariant that noise*g is "round."
...
embedCT ct = let (CT k _ l c) = absorbGFactors ct
              in CT k 0 l (embed <$> c)
...
twaceCT :: (r `Divides` r', s' `Divides` r', s ~ (FGCD s' r), ExtensionCyc c zq, AbsorbGCtx c r' zp zq)
           => CT r zp (c r' zq) -> CT s zp (c s' zq)
twaceCT ct = let (CT k _ l c) = absorbGFactors ct
              in CT k 0 l (twace <$> c)
