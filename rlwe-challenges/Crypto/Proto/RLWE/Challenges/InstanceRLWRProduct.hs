...
module Crypto.Proto.RLWE.Challenges.InstanceRLWRProduct (InstanceRLWRProduct(..)) where
import Prelude ((+), (/))
...
import qualified Crypto.Proto.Crypto.Proto.RLWE.SampleRLWRProduct as Crypto.Proto.RLWE (SampleRLWRProduct)
import qualified Crypto.Proto.RLWE.Challenges.RLWRParams as Crypto.Proto.RLWE.Challenges (RLWRParams)

data InstanceRLWRProduct = InstanceRLWRProduct{challengeID :: !(P'.Int32), instanceID :: !(P'.Int32),
                                               params :: !(Crypto.Proto.RLWE.Challenges.RLWRParams),
                                               samples :: !(P'.Seq Crypto.Proto.RLWE.SampleRLWRProduct)}
                         deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable InstanceRLWRProduct where
  mergeAppend (InstanceRLWRProduct x'1 x'2 x'3 x'4) (InstanceRLWRProduct y'1 y'2 y'3 y'4)
   = InstanceRLWRProduct (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3) (P'.mergeAppend x'4 y'4)

instance P'.Default InstanceRLWRProduct where
  defaultValue = InstanceRLWRProduct P'.defaultValue P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire InstanceRLWRProduct where
  wireSize ft' self'@(InstanceRLWRProduct x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(InstanceRLWRProduct x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> put'Fields
       11 -> do
