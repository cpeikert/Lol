...
module Crypto.Challenges.RLWE.Proto.RLWE.SampleRound (SampleRound(..)) where
import Prelude ((+), (/))
...
data SampleRound = SampleRound{a :: !(Lol.Rq), b :: !(Lol.Rq)}
                 deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable SampleRound where
  mergeAppend (SampleRound x'1 x'2) (SampleRound y'1 y'2) = SampleRound (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)

instance P'.Default SampleRound where
  defaultValue = SampleRound P'.defaultValue P'.defaultValue

instance P'.Wire SampleRound where
  wireSize ft' self'@(SampleRound x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(SampleRound x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
