   ...
   = case ft' of
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             8 -> Prelude'.fmap (\ !new'Field -> old'Self{instID = new'Field}) (P'.wireGet 5)
...
instance P'.TextMsg Secret where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'instID, parse'm, parse'q, parse's]) P'.spaces
