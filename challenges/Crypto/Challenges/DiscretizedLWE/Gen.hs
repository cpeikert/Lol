...
module Crypto.Challenges.DiscretizedLWE.Gen
(rlweInstance
...
rlweInstance :: (LWECtx t m z zq v q, MonadRandom rnd, Random z)
  => v -> Int -> TaggedT q rnd (Cyc t m z, [RLWESampleDisc t m zq])
...
rlweSample :: (MonadRandom rnd, LWECtx t m z zq v q)
          => v -> Cyc t m z -> TaggedT q rnd (RLWESampleDisc t m zq)
rlweSample svar s = do
  let sq = reduce s
  e <- errorRounded svar
  a <- tagT $ adviseCRT <$> getRandom
  return $ RLWESampleDisc a $ a * sq + reduce (e `asTypeOf` s)
