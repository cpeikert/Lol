...
void pNormSqD (double* y, hShort_t tupSize, hDim_t lts, hDim_t rts, hDim_t p) {
  hDim_t blockOffset;
  hDim_t modOffset;
  hDim_t i;
...
void ppNormSqD (void* y, hShort_t tupSize, PrimeExponent pe, hDim_t lts, hDim_t rts, hInt_t* qs) {
#ifdef DEBUG_MODE
  ASSERT (q==0);
#endif
    hDim_t p = pe.prime;
    hShort_t e = pe.exponent;
    for(int tupIdx = 0; tupIdx < tupSize; tupIdx++) {
      pNormSqD (((double*)y)+tupIdx, tupSize, lts*ipow(p,e-1), rts, p);
    }
}
...
extern "C" void tensorNormSqD (hShort_t tupSize, double* y, hDim_t totm, PrimeExponent* peArr, hShort_t sizeOfPE) {
  ...
  tensorFuser(y, tupSize, ppNormSqD, totm, peArr, sizeOfPE, (hInt_t*)0);
