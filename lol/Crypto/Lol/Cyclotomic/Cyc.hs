...
type CElt t r =
    ( UCRTElt t r
    , IntegralDomain r, IntegralDomain (TRep t r)
    , ZeroTestable r, ZeroTestable (TRep t r)
    )
...
-- | Generate an LWE error term with given scaled variance, deterministically
-- rounded with respect to the decoding basis.
--
-- NOTE: This implementation uses Double precision to generate the Gaussian
...
-- | Generate an LWE error term with given scaled variance @* p^2@ over the
-- given coset, deterministically rounded with respect to the decoding basis.
--
-- NOTE: This implementation uses Double precision to generate the Gaussian
-- sample, which may not be sufficient for rigorous proof-based security.
--
...
twace (Sub (c :: Cyc t l r)) =
  Sub (twace c :: Cyc t (FGCD l m) r)
...
crtSet :: ( m `Divides` m', ZPP r, CElt t r, TElt t (ZpOf r)
          , TRep t (ZpOf r) ~ ZpOf (TRep t r), ZPP (TRep t r)
          )
       => Tagged m [Cyc t m' r]
crtSet = (Pow <$>) <$> U.crtSet
{-# INLINABLE crtSet #-}
...
liftCyc
    :: (Lift b a, Lift (TRep t b) (TRep t a), Fact m, TElt t a, CElt t b)
    => R.Basis
    -> Cyc t m b
    -> Cyc t m a
{-# INLINABLE liftCyc #-}
...
toPow' :: (Fact m, CElt t r) => Cyc t m r -> Cyc t m r
{-# INLINE toPow' #-}
toPow' c@(Pow _) = c
...
toDec' :: (Fact m, CElt t r) => Cyc t m r -> Cyc t m r
{-# INLINE toDec' #-}
...
toCRT' :: (Fact m, CElt t r) => Cyc t m r -> Cyc t m r
{-# INLINE toCRT' #-}
