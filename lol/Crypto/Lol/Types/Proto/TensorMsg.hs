...
import qualified Crypto.Lol.Types.Proto.Coeffs as Lol.TensorMsg.Coeffs (Coeffs(..), get'zs, get'zqs)
    ...
    where
        put'Fields
         = do
             ...
             P'.wirePutOpt 26 11 (Lol.TensorMsg.Coeffs.get'zqs Prelude'.=<< x'2)
  wireGet ft'
   = case ft' of
  ...
  textGet
   = do
    ...
    where
        ...
        parse'coeffs = P'.try (P'.choice [parse'zs, parse'zqs])
          where
              parse'zs
               = P'.try
                  (do
