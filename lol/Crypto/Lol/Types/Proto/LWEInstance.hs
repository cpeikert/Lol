...
data LWEInstance = LWEInstance{id :: !(P'.Word32), svar :: !(P'.Double), samples :: !(P'.Seq Lol.LWESample)}
                 deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)

instance P'.Mergeable LWEInstance where
  mergeAppend (LWEInstance x'1 x'2 x'3) (LWEInstance y'1 y'2 y'3)
   = LWEInstance (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)

instance P'.Default LWEInstance where
  defaultValue = LWEInstance P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire LWEInstance where
  wireSize ft' self'@(LWEInstance x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeReq 1 13 x'1 + P'.wireSizeReq 1 1 x'2 + P'.wireSizeRep 1 11 x'3)
  wirePut ft' self'@(LWEInstance x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
...
instance P'.TextMsg LWEInstance where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'id, parse'svar, parse'samples]) P'.spaces
