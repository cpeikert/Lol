...
module Crypto.Proto.RLWE.SampleContLegacy (SampleContLegacy(..)) where
import Prelude ((+), (/))
...
data SampleContLegacy = SampleContLegacy{a :: !(RLWE.Rq), b :: !(RLWE.Kq)}
                      deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable SampleContLegacy where
  mergeAppend (SampleContLegacy x'1 x'2) (SampleContLegacy y'1 y'2)
   = SampleContLegacy (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)

instance P'.Default SampleContLegacy where
  defaultValue = SampleContLegacy P'.defaultValue P'.defaultValue

instance P'.Wire SampleContLegacy where
  wireSize ft' self'@(SampleContLegacy x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(SampleContLegacy x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
