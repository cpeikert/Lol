...
module Crypto.Proto.RLWE.SampleDiscLegacy (SampleDiscLegacy(..)) where
import Prelude ((+), (/))
...
data SampleDiscLegacy = SampleDiscLegacy{a :: !(RLWE.Rq), b :: !(RLWE.Rq)}
                      deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable SampleDiscLegacy where
  mergeAppend (SampleDiscLegacy x'1 x'2) (SampleDiscLegacy y'1 y'2)
   = SampleDiscLegacy (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)

instance P'.Default SampleDiscLegacy where
  defaultValue = SampleDiscLegacy P'.defaultValue P'.defaultValue

instance P'.Wire SampleDiscLegacy where
  wireSize ft' self'@(SampleDiscLegacy x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(SampleDiscLegacy x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
