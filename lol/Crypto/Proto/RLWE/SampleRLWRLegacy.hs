...
module Crypto.Proto.RLWE.SampleRLWRLegacy (SampleRLWRLegacy(..)) where
import Prelude ((+), (/))
...
data SampleRLWRLegacy = SampleRLWRLegacy{a :: !(RLWE.Rq), b :: !(RLWE.Rq)}
                      deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

instance P'.Mergeable SampleRLWRLegacy where
  mergeAppend (SampleRLWRLegacy x'1 x'2) (SampleRLWRLegacy y'1 y'2)
   = SampleRLWRLegacy (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)

instance P'.Default SampleRLWRLegacy where
  defaultValue = SampleRLWRLegacy P'.defaultValue P'.defaultValue

instance P'.Wire SampleRLWRLegacy where
  wireSize ft' self'@(SampleRLWRLegacy x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(SampleRLWRLegacy x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
